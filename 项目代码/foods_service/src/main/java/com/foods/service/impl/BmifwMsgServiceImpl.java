...
@Service
public class BmifwMsgServiceImpl implements BmifwMsgService{
	...
	*/
	...
	public String add(BmifwMsg model,LoginModel login){
		...
		bmifwMsgMapper.insertSelective(model);//插入数据
		return "";
	}
	/**
	修改
	*/
	...
	public String update(BmifwMsg model,LoginModel login){
		BmifwMsg preModel = bmifwMsgMapper.selectByPrimaryKey(model.getId());
		if(model.getFwName()==null||model.getFwName().equals("")){
			return "范围名为必填属性";
		}
		if(model.getMin()==null){
		...
		bmifwMsgMapper.updateByPrimaryKey(preModel);//更新数据
		return "";
	}
	/**
	*根据参数查询BMI范围列表总数
	*/
	...
	public Map<String,Integer> getDataListCount(BmifwMsg queryModel,Integer pageSize,LoginModel login) {
		BmifwMsgExample se = new BmifwMsgExample();
		BmifwMsgExample.Criteria sc = se.createCriteria();
		...
		int count = (int)bmifwMsgMapper.countByExample(se);
		int totalPage = 0;
	...
	*/
	...
	public Map<String,Object> getDataList(BmifwMsg queryModel,Integer page,Integer pageSize,LoginModel login) {
		...
		List<BmifwMsg> list = bmifwMsgMapper.selectByExample(se);//执行查询语句
		Map<String,Object> rs = new HashMap<String,Object>();
		List<Map<String,Object>> list2 = new ArrayList<Map<String,Object>>();
		for(BmifwMsg model:list){
			list2.add(getBmifwMsgModel(model,login));
		}
	...
	*/
	...
	public void delete(Integer id) {
		bmifwMsgMapper.deleteByPrimaryKey(id);
	}
}
