	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="运动类型" style="min-width:250px;">
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/user/yd_log/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="运动日期">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydLog.ydDate==null?'':row.ydLog.ydDate)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="运动分钟数">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydLog.ydfzs==null?'':row.ydLog.ydfzs)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="运动类型">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydTypeStr==null?'':row.ydTypeStr)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="每分钟消耗">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydLog.mfzxh==null?'':row.ydLog.mfzxh)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="总热量">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydLog.zrNum==null?'':row.ydLog.zrNum)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="用户ID">
						...
						<el-table-column
							...
							<template v-slot="{ row }">
								<el-button
									href="javascript:void(0)" :data-id="row.ydLog.id" @click="del"
									type="danger" style="margin-left:5px;margin-top:5px;"
									>
									删除
								</el-button>
								<el-button
									href="javascript:void(0)" :data-params="'id='+row.ydLog.id+''" data-href="/user/yd_log/update_page" @click="goToPage"
					...
					<div>
						<div id="yd_log_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</el-card>
...
export default {
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				...
				success : function(result) {
					...
					that.ydTypeMsgList = admin_utils.addAllSelect(result.ydTypeMsgList);
				}
			});
		},
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/user/yd_log/del",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/yd_log/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"ydType":ydType,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/yd_log/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
