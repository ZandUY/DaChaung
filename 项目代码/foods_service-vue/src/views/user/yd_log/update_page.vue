...
<template>
	<div class="edit-page-div">
		<div class="box-card">
			<h2 class="edit-page-div-h2">运动记录-修改</h2>
...
export default {
	...
	mounted() {
		...
		date_utils.setDomVal("ydDate",that.ydDate);
		setSelectableVal('ydType',that.ydType)
	},
	methods:{
		getDataList(){
			var that = this;
		...
		queryDataDetail(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/user/yd_log/queryDataDetail",
				async: false,
				data : {
					'id':that.id,
					'loginToken':that.currentLoginToken,
				...
				success : function(result) {
					that.ydDate=result.ydDate;
					that.ydfzs=result.ydfzs;
					that.ydType=result.ydType;
					that.zrNum=result.zrNum;
				}
			});
		},
		submitData(){
			...
			$.ajax({
				type : 'post',
				url : this.REQUEST_URL+"/user/yd_log/update_submit",
				async: false,
				data : {
					'id':that.id,
					'ydDate':ydDate,
