	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="运动计划" style="min-width:250px;">
					<select  class="" data-id="ydjh"    id="ydjh" >
						<template v-for="item in ydjhMsgList">
			...
			<div >
				<el-table
					...
					<el-table-column
						label="运动计划">
						<template v-slot="{ row }">
							<a href="javascript:void(0)"   :data-targetpage="'/index/ydjh_detail?id='+row.ydCollectMsg.ydjh+''"  data-href="/index" @click="goToChildPageWithBlank">
								<div v-html="''+(row.ydjhStr==null?'':row.ydjhStr)+''"></div>
							</a>
						</template>
					</el-table-column>
					<el-table-column
						...
						<el-table-column
							label="收藏时间">
							<template v-slot="{ row }">
								<div v-html="''+(row.ydCollectMsg.collectTime==null?'':row.ydCollectMsg.collectTime)+''"></div>
							</template>
						</el-table-column>
					</el-table>
					<div>
						<div id="yd_collect_msg_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</el-card>
...
export default {
	...
	mounted() {
		...
		that.getInitData();
		$('#ydjh').next().remove();
		$('#ydjh').searchableSelect(that.ydjhMsgList);
		$('#userId').next().remove();
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/user/yd_collect_msg/getInitData",
				async: false,
				data : {
				...
				success : function(result) {
					...
					that.ydjhMsgList = admin_utils.addAllSelect(result.ydjhMsgList);
				}
			});
		},
		ajaxCount(){
			var that = this;
			var rs = null;
			var ydjh = getSelectableVal('ydjh');
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/yd_collect_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"ydjh":ydjh,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/yd_collect_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
