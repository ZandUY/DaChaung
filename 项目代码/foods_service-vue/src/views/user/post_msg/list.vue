	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				...
				<el-form-item label="副标题" style="min-width:250px;">
					<el-input type="text"  placeholder="请输入副标题" name="subtitle" v-model="subtitle" class="" id="subtitle"></el-input>
				</el-form-item>
				<el-form-item>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/user/post_msg/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="标题">
						<template v-slot="{ row }">
							<div v-html="''+(row.postMsg.title==null?'':row.postMsg.title)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="副标题">
						<template v-slot="{ row }">
							<div v-html="''+(row.postMsg.subtitle==null?'':row.postMsg.subtitle)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="用户ID">
						...
						<el-table-column
							label="发布时间">
							<template v-slot="{ row }">
								<div v-html="''+(row.postMsg.fbTime==null?'':row.postMsg.fbTime)+''"></div>
							</template>
						</el-table-column>
						<el-table-column
							fixed="right"
							...
							<template v-slot="{ row }">
								<el-button
									href="javascript:void(0)" :data-params="'id='+row.postMsg.id+''" data-href="/user/post_msg/detail" @click="goToPage"
									type="success" style="margin-left:5px;margin-top:5px;"
									>
									查看详情
								</el-button>
								<el-button
									href="javascript:void(0)" :data-id="row.postMsg.id" @click="del1"
									type="danger" style="margin-left:5px;margin-top:5px;"
									>
									删除
								</el-button>
								<el-button
									href="javascript:void(0)" :data-params="'id='+row.postMsg.id+''" data-href="/user/post_msg/update_page" @click="goToPage"
					...
					<div>
						<div id="post_msg_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</el-card>
	...
	methods:{
		del1(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/user/post_msg/del1",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/post_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"title":title,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/post_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
