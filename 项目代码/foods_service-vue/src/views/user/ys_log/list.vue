	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="饮食时间" style="min-width:250px;">
					<el-select  class="" data-id="ysTime"  v-model="ysTime"  id="ysTime" >
						<template v-for="item in ysTimeList">
							<el-option  :value="item.id"   :label="item.name"></el-option>
						</template>
					</el-select>
				</el-form-item>
				<el-form-item label="饮食类型" style="min-width:250px;">
					<select  class="" data-id="ysType"    id="ysType" >
						<template v-for="item in ysTypeMsgList">
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/user/ys_log/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="饮食日期">
						<template v-slot="{ row }">
							<div v-html="''+(row.ysLog.ysDate==null?'':row.ysLog.ysDate)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="饮食时间">
						<template v-slot="{ row }">
							<div v-html="''+(row.ysTimeStr==null?'':row.ysTimeStr)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="饮食类型">
						<template v-slot="{ row }">
							<div v-html="''+(row.ysTypeStr==null?'':row.ysTypeStr)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="每千克热量">
						<template v-slot="{ row }">
							<div v-html="''+(row.ysLog.mqkrNum==null?'':row.ysLog.mqkrNum)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="用户ID">
						...
						<el-table-column
							label="总热量">
							<template v-slot="{ row }">
								<div v-html="''+(row.ysLog.zrl==null?'':row.ysLog.zrl)+''"></div>
							</template>
						</el-table-column>
						<el-table-column
							fixed="right"
							...
							<template v-slot="{ row }">
								<el-button
									href="javascript:void(0)" :data-id="row.ysLog.id" @click="del"
									type="danger" style="margin-left:5px;margin-top:5px;"
									>
									删除
								</el-button>
								<el-button
									href="javascript:void(0)" :data-params="'id='+row.ysLog.id+''" data-href="/user/ys_log/update_page" @click="goToPage"
					...
					<div>
						<div id="ys_log_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</el-card>
...
export default {
	...
	mounted() {
		...
		that.getInitData();
		$('#ysType').next().remove();
		$('#ysType').searchableSelect(that.ysTypeMsgList);
		that.ajaxList(1);
	},
	methods:{
		getInitData(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/user/ys_log/getInitData",
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
				},
				success : function(result) {
					that.ysTypeMsgList = admin_utils.addAllSelect(result.ysTypeMsgList);
					that.ysTimeList = admin_utils.addAllSelect(result.ysTimeList);
				}
			});
		},
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/user/ys_log/del",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		ajaxCount(){
			var that = this;
			var rs = null;
			var ysTime = that.ysTime;
			var ysType = getSelectableVal('ysType');
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/ys_log/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"ysTime":ysTime,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/ys_log/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
