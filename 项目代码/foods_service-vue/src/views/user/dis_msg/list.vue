	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="评论内容" style="min-width:250px;">
					<el-input type="text"  placeholder="请输入评论内容" name="disContent" v-model="disContent" class="" id="disContent"></el-input>
				</el-form-item>
				<div style="display:inline-block">
					<el-form-item label="评论时间" style="min-width:200px;">
						<input type="date" id="disTime1"  class="el-input__inner"  style="padding:4px;border:1px solid #409eff;border-radius: var(--el-input-border-radius,var(--el-border-radius-base)); ">
					</el-form-item>
					<el-form-item style="min-width:200px;">
						<input type="date" id="disTime2"  class="el-input__inner"  style="padding:4px;border:1px solid #409eff;border-radius: var(--el-input-border-radius,var(--el-border-radius-base)); ">
					</el-form-item>
				</div>
				<el-form-item>
			...
			<div >
				<el-table
					...
					<el-table-column
						label="帖子ID">
						<template v-slot="{ row }">
							<a href="javascript:void(0)"   :data-targetpage="'/index/post_detail?id='+row.disMsg.postId+''"  data-href="/index" @click="goToChildPageWithBlank">
								<div v-html="''+(row.postIdStr==null?'':row.postIdStr)+''"></div>
							</a>
						</template>
					</el-table-column>
					<el-table-column
						label="评论内容">
						<template v-slot="{ row }">
							<div v-html="''+(row.disMsg.disContent==null?'':row.disMsg.disContent)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="用户ID">
						...
						<el-table-column
							label="评论时间">
							<template v-slot="{ row }">
								<div v-html="''+(row.disMsg.disTime==null?'':row.disMsg.disTime)+''"></div>
							</template>
						</el-table-column>
						<el-table-column
							fixed="right"
							...
							<template v-slot="{ row }">
								<el-button
									href="javascript:void(0)" :data-id="row.disMsg.id" @click="del1"
					...
					<div>
						<div id="dis_msg_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</el-card>
	...
	methods:{
		del1(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/user/dis_msg/del1",
					async: false,
					data : {
		...
		ajaxCount(){
			var that = this;
			var rs = null;
			var disContent = that.disContent;
			var disTime1 = date_utils.getDomVal("disTime1");
			var disTime2 = date_utils.getDomVal("disTime2");
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/dis_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/dis_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
