	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				...
				<el-form-item label="留言状态" style="min-width:250px;">
					<el-select  class="" data-id="lmsgStatus"  v-model="lmsgStatus"  id="lmsgStatus" >
						<template v-for="item in lmsgStatusList">
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/user/lmsg_suggest_msg/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="标题">
						<template v-slot="{ row }">
							<div v-html="''+(row.lmsgSuggestMsg.title==null?'':row.lmsgSuggestMsg.title)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="留言内容">
						<template v-slot="{ row }">
							<div v-html="''+(row.lmsgSuggestMsg.lmsgContent==null?'':row.lmsgSuggestMsg.lmsgContent)+''"></div>
						</template>
					</el-table-column>
					<el-table-column label="留言附图"  width="200px">
						<template v-slot="{ row }">
							<template v-for="item4 in row.lmsgImgList">
						...
						<el-table-column
							...
							<el-table-column
								label="留言状态">
								<template v-slot="{ row }">
									<div v-html="''+(row.lmsgStatusStr==null?'':row.lmsgStatusStr)+''"></div>
								</template>
							</el-table-column>
							<el-table-column
								label="回复内容">
								<template v-slot="{ row }">
									<div v-html="''+(row.lmsgSuggestMsg.replyContent==null?'':row.lmsgSuggestMsg.replyContent)+''"></div>
								</template>
							</el-table-column>
							<el-table-column
								label="留言时间">
								<template v-slot="{ row }">
									<div v-html="''+(row.lmsgSuggestMsg.lmsgTime==null?'':row.lmsgSuggestMsg.lmsgTime)+''"></div>
								</template>
							</el-table-column>
							<el-table-column
								fixed="right"
								...
								<template v-slot="{ row }">
									<el-button
										href="javascript:void(0)" :data-id="row.lmsgSuggestMsg.id" @click="del1"
										type="danger" style="margin-left:5px;margin-top:5px;"
										>
										删除
									</el-button>
									<template v-if="row.lmsgSuggestMsg.lmsgStatus=='1'">
										<el-button
											href="javascript:void(0)" :data-params="'id='+row.lmsgSuggestMsg.id+''" data-href="/user/lmsg_suggest_msg/update_page" @click="goToPage"
						...
						<div>
							<div id="lmsg_suggest_msg_bar" class="pagination" style="margin-top:20px;"></div>
						</div>
					</div>
				</div>
			</el-card>
...
export default {
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/user/lmsg_suggest_msg/getInitData",
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
				},
				success : function(result) {
					that.lmsgStatusList = admin_utils.addAllSelect(result.lmsgStatusList);
				}
			});
		},
		del1(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/user/lmsg_suggest_msg/del1",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		ajaxCount(){
			...
			var lmsgStatus = that.lmsgStatus;
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/lmsg_suggest_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"title":title,
					"lmsgStatus":lmsgStatus,
				},
				success : function(result) {
					rs = result;
				}
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/lmsg_suggest_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
