	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="饮食计划" style="min-width:250px;">
					<select  class="" data-id="ysjh"    id="ysjh" >
						<template v-for="item in ysjhMsgList">
							<option :value="item.id">{{item.name}}</option>
						</template>
					</select>
				</el-form-item>
				<el-form-item label="用户ID" style="min-width:250px;">
					<select  class="" data-id="userId"    id="userId" >
						<template v-for="item in userMsgList">
							<option :value="item.id">{{item.name}}</option>
						</template>
					</select>
				</el-form-item>
				<div style="display:inline-block">
					<el-form-item label="收藏时间" style="min-width:200px;">
						<input type="date" id="collectTime1"  class="el-input__inner"  style="padding:4px;border:1px solid #409eff;border-radius: var(--el-input-border-radius,var(--el-border-radius-base)); ">
					</el-form-item>
					<el-form-item style="min-width:200px;">
						<input type="date" id="collectTime2"  class="el-input__inner"  style="padding:4px;border:1px solid #409eff;border-radius: var(--el-input-border-radius,var(--el-border-radius-base)); ">
					</el-form-item>
				</div>
				<el-form-item>
			...
			<div >
				<el-table
					...
					<el-table-column
						label="饮食计划">
						<template v-slot="{ row }">
							<a href="javascript:void(0)"   :data-targetpage="'/index/ysjh_detail?id='+row.ysCollectMsg.ysjh+''"  data-href="/index" @click="goToChildPageWithBlank">
								<div v-html="''+(row.ysjhStr==null?'':row.ysjhStr)+''"></div>
							</a>
						</template>
					</el-table-column>
					<el-table-column
						...
						<template v-slot="{ row }">
							<div>
								<img :src="row.userIdImg" style="height:40px;width:40px;border-radius:100%">
								</div>
						...
						<el-table-column
							label="收藏时间">
							<template v-slot="{ row }">
								<div v-html="''+(row.ysCollectMsg.collectTime==null?'':row.ysCollectMsg.collectTime)+''"></div>
							</template>
						</el-table-column>
					</el-table>
					<div>
						<div id="ys_collect_msg_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</el-card>
...
export default {
	...
	mounted() {
		...
		that.getInitData();
		$('#ysjh').next().remove();
		$('#ysjh').searchableSelect(that.ysjhMsgList);
		$('#userId').next().remove();
		$('#userId').searchableSelect(that.userMsgList);
		that.ajaxList(1);
	},
	methods:{
		getInitData(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/user/ys_collect_msg/getInitData",
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
				},
				success : function(result) {
					that.userMsgList = admin_utils.addAllSelect(result.userMsgList);
					that.ysjhMsgList = admin_utils.addAllSelect(result.ysjhMsgList);
				}
			});
		},
		ajaxCount(){
			var that = this;
			var rs = null;
			var ysjh = getSelectableVal('ysjh');
			var userId = getSelectableVal('userId');
			var collectTime1 = date_utils.getDomVal("collectTime1");
			var collectTime2 = date_utils.getDomVal("collectTime2");
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/ys_collect_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"ysjh":ysjh,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/user/ys_collect_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
