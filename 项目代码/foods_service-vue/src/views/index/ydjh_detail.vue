	...
	<div style="padding:20px;width:80%;margin-left:10%">
		<div style="padding:20px;padding-left:50px;padding-right:50px;border-radius: 10px;margin-top:10px;background-color: white;min-height:350px;">
			...
			<div style="display:flex">
				<div style="flex:2">
					<div style="margin-top:20px;">
						<img :src="''+ydjhMsgDetail.model.fmImg+''" style="width: 250px; height: 250px;border-radius: 10px;">
						</div>
					</div>
					<div style="flex:10;margin-left:20px;">
						<div style="margin-top:20px;margin-left:30px;">
							<h2 style="margin-bottom:20px;">{{ydjhMsgDetail.model.jhName}}</h2>
							<div style="display:flex;margin-top:20px;">
								<div style="flex:3;color:gray">BMI范围：</div>
								<div style="flex:11;color:gray">
									{{ydjhMsgDetail.bmifwT.fwName}}
								</div>
							</div>
							<div style="display:flex;margin-top:20px;">
								<div style="flex:3;color:gray">运动消耗范围：</div>
								<div style="flex:11;color:gray">
									{{ydjhMsgDetail.ydxhfwT.fwName}}
								</div>
							</div>
							<div style="display:flex;margin-top:20px;">
								<div style="flex:3;color:gray">发布时间：</div>
								<div style="flex:11;color:gray">
									{{ydjhMsgDetail.model.fbTime}}
								</div>
							</div>
						</div>
						<div style="margin-top:20px;">
								...
								<div style="margin-top:20px">
									<span >
										<template v-if="isCollect==1">
											<button @click="submitCollect" class="el-button"   style="margin-left:5px;">取消运动收藏</button>
										</template>
										<template v-if="isCollect==0">
											<button @click="submitCollect" class="el-button el-button--primary" style="margin-left:5px;">运动收藏</button>
										</template>
									</span>
								</div>
								<div style="margin-top:20px;">
									<div style="margin-bottom:20px;margin-top:20px;width:100%">
										<h3 style="margin-bottom:10px;color:#f1bc6c;text-align:left">计划详情</h3>
										<el-divider></el-divider>
										<div style="padding:20px;" v-html="''+ydjhMsgDetail.model.jhDetail+''">
										</div>
									</div>
								</div>
							</div>
...
export default {
	...
	methods:{
		submitCollect(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/index/ydjh_detail/submitCollect",
				async: false,
				data : {
		...
		initData(){
			...
			$.ajax({
				type : 'get',
				url : that.REQUEST_URL+"/index/ydjh_detail/getData",
				async: false,
				data : {
					id:that.id,
					"loginToken":that.currentLoginToken,
				...
				success : function(result) {
					that.ydjhMsgDetail = result.ydjhMsgDetail;
