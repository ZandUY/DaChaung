	...
	<div style="padding:20px;width:80%;margin-left:10%">
		<div style="padding:20px;padding-left:50px;padding-right:50px;border-radius: 10px;margin-top:10px;background-color: white;min-height:350px;">
			...
			<div style="display:flex">
				<div style="flex:2">
					<div style="margin-top:20px;">
						<img :src="''+ysjhMsgDetail.model.fmImg+''" style="width: 250px; height: 250px;border-radius: 10px;">
						</div>
					</div>
					<div style="flex:10;margin-left:20px;">
						<div style="margin-top:20px;margin-left:30px;">
							<h2 style="margin-bottom:20px;">{{ysjhMsgDetail.model.jhName}}</h2>
							<div style="display:flex;margin-top:20px;">
								<div style="flex:3;color:gray">BMI范围：</div>
								<div style="flex:11;color:gray">
									{{ysjhMsgDetail.bmifwT.fwName}}
								</div>
							</div>
							<div style="display:flex;margin-top:20px;">
								<div style="flex:3;color:gray">饮食热量范围：</div>
								<div style="flex:11;color:gray">
									{{ysjhMsgDetail.ynumFwT.fwName}}
								</div>
							</div>
							<div style="display:flex;margin-top:20px;">
								<div style="flex:3;color:gray">发布时间：</div>
								<div style="flex:11;color:gray">
									{{ysjhMsgDetail.model.fbTime}}
								</div>
							</div>
						</div>
						<div style="margin-top:20px;">
								...
								<div style="margin-top:20px">
									<span >
										<template v-if="isCollect==1">
											<button @click="submitCollect" class="el-button"   style="margin-left:5px;">取消饮食收藏</button>
										</template>
										<template v-if="isCollect==0">
											<button @click="submitCollect" class="el-button el-button--primary" style="margin-left:5px;">饮食收藏</button>
										</template>
									</span>
								</div>
								<div style="margin-top:20px;">
									<div style="margin-bottom:20px;margin-top:20px;width:100%">
										...
										<el-divider></el-divider>
										<div style="padding:20px;" v-html="''+ysjhMsgDetail.model.jhDetail+''">
										</div>
									</div>
								</div>
							</div>
...
export default {
	...
	methods:{
		submitCollect(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/index/ysjh_detail/submitCollect",
				async: false,
				data : {
		...
		initData(){
			...
			$.ajax({
				type : 'get',
				url : that.REQUEST_URL+"/index/ysjh_detail/getData",
				async: false,
				data : {
					id:that.id,
					"loginToken":that.currentLoginToken,
				...
				success : function(result) {
					that.ysjhMsgDetail = result.ysjhMsgDetail;
