...
<template>
	<div class="edit-page-div">
		<div class="box-card">
			<h2 class="edit-page-div-h2">营养小常识-修改</h2>
			...
			<div class="edit-form-div" id="csContentForm">
				<div  class="edit-form-name">常识内容</div>
				<div class="edit-form-div2">
					<div class="edit-form-div3">
						<div class="editorDiv" id="csContent" data-id="csContent"></div>
					</div>
				</div>
			</div>
...
export default {
	...
	mounted() {
		...
		initAllEditor(this.REQUEST_URL,'csContent');
		that.queryDataDetail();
		setZoneImg(fmImgDrop,that.fmImg);
		setEditorHtml('csContent',that.csContent)
	},
	methods:{
		queryDataDetail(){
			var that = this;
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/yyxcs_msg/queryDataDetail",
				async: false,
				data : {
					'id':that.id,
				...
				success : function(result) {
					...
					that.csContent=result.csContent;
					setEditorHtml('csContent',result.csContent)
				}
			});
		},
		submitData(){
			...
			var csContent = getEditVal('csContent');
			$.ajax({
				type : 'post',
				url : this.REQUEST_URL+"/admin/yyxcs_msg/update_submit",
				async: false,
				data : {
					'id':that.id,
					'title':title,
