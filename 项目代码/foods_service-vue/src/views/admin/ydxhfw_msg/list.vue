	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/admin/ydxhfw_msg/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="范围名">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydxhfwMsg.fwName==null?'':row.ydxhfwMsg.fwName)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="最小值">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydxhfwMsg.min==null?'':row.ydxhfwMsg.min)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="最大值">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydxhfwMsg.max==null?'':row.ydxhfwMsg.max)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						fixed="right"
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-id="row.ydxhfwMsg.id" @click="del"
								type="danger" style="margin-left:5px;margin-top:5px;"
								>
								删除
							</el-button>
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.ydxhfwMsg.id+''" data-href="/admin/ydxhfw_msg/update_page" @click="goToPage"
				...
				<div>
					<div id="ydxhfw_msg_bar" class="pagination" style="margin-top:20px;"></div>
				</div>
			</div>
		</div>
	</el-card>
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/ydxhfw_msg/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/ydxhfw_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"fwName":fwName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/ydxhfw_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
