	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/admin/bmifw_msg/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="范围名">
						<template v-slot="{ row }">
							<div v-html="''+(row.bmifwMsg.fwName==null?'':row.bmifwMsg.fwName)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="最小值">
						<template v-slot="{ row }">
							<div v-html="''+(row.bmifwMsg.min==null?'':row.bmifwMsg.min)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="最大值">
						<template v-slot="{ row }">
							<div v-html="''+(row.bmifwMsg.max==null?'':row.bmifwMsg.max)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						fixed="right"
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-id="row.bmifwMsg.id" @click="del"
								type="danger" style="margin-left:5px;margin-top:5px;"
								>
								删除
							</el-button>
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.bmifwMsg.id+''" data-href="/admin/bmifw_msg/update_page" @click="goToPage"
				...
				<div>
					<div id="bmifw_msg_bar" class="pagination" style="margin-top:20px;"></div>
				</div>
			</div>
		</div>
	</el-card>
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/bmifw_msg/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/bmifw_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"fwName":fwName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/bmifw_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
