	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="回复内容" style="min-width:250px;">
					<el-input type="text"  placeholder="请输入回复内容" name="replyContent" v-model="replyContent" class="" id="replyContent"></el-input>
				</el-form-item>
				<div style="display:inline-block">
					<el-form-item label="回复时间" style="min-width:200px;">
						<input type="date" id="replyTime1"  class="el-input__inner"  style="padding:4px;border:1px solid #409eff;border-radius: var(--el-input-border-radius,var(--el-border-radius-base)); ">
					</el-form-item>
					<el-form-item style="min-width:200px;">
						<input type="date" id="replyTime2"  class="el-input__inner"  style="padding:4px;border:1px solid #409eff;border-radius: var(--el-input-border-radius,var(--el-border-radius-base)); ">
					</el-form-item>
				</div>
				<el-form-item>
			...
			<div >
				<el-table
					...
					<el-table-column
						label="评论ID">
						<template v-slot="{ row }">
							<div v-html="''+(row.disIdStr==null?'':row.disIdStr)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="回复内容">
						<template v-slot="{ row }">
							<div v-html="''+(row.replyMsg.replyContent==null?'':row.replyMsg.replyContent)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="用户ID">
						...
						<el-table-column
							label="回复时间">
							<template v-slot="{ row }">
								<div v-html="''+(row.replyMsg.replyTime==null?'':row.replyMsg.replyTime)+''"></div>
							</template>
						</el-table-column>
						<el-table-column
							fixed="right"
							...
							<template v-slot="{ row }">
								<el-button
									href="javascript:void(0)" :data-id="row.replyMsg.id" @click="del"
					...
					<div>
						<div id="reply_msg_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</el-card>
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/reply_msg/del",
					async: false,
					data : {
		...
		ajaxCount(){
			var that = this;
			var rs = null;
			var replyContent = that.replyContent;
			var replyTime1 = date_utils.getDomVal("replyTime1");
			var replyTime2 = date_utils.getDomVal("replyTime2");
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/reply_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"replyContent":replyContent,
					"replyTime1":replyTime1,
					"replyTime2":replyTime2,
				},
				success : function(result) {
					rs = result;
				}
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/reply_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
