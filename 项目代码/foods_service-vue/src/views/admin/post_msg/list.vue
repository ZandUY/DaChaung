				...
				<el-table
					...
					<el-table-column
						...
						<el-table-column
							...
							<template v-slot="{ row }">
								<el-button
									href="javascript:void(0)" :data-params="'id='+row.postMsg.id+''" data-href="/admin/post_msg/detail" @click="goToPage"
									type="success" style="margin-left:5px;margin-top:5px;"
									>
									查看详情
								</el-button>
								<el-button
									href="javascript:void(0)" :data-id="row.postMsg.id" @click="del"
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/post_msg/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/post_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"title":title,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/post_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
