...
<template>
	<div class="edit-page-div">
		<div class="box-card">
			<h2 class="edit-page-div-h2">系统介绍-修改</h2>
			...
			<div class="edit-form-div" id="introDetailForm">
				<div  class="edit-form-name">介绍详情</div>
				<div class="edit-form-div2">
					<div class="edit-form-div3">
						<div class="editorDiv" id="introDetail" data-id="introDetail"></div>
					</div>
				</div>
			</div>
			...
			<div class="edit-form-div" id="addressForm">
				<div  class="edit-form-name">地址</div>
				<div class="edit-form-div2">
					<div class="edit-form-div3">
...
export default {
	...
	mounted() {
		...
		initAllEditor(this.REQUEST_URL,'introDetail');
		that.queryDataDetail();
		setZoneImg(fmImgDrop,that.fmImg);
		setEditorHtml('introDetail',that.introDetail)
	},
	methods:{
		queryDataDetail(){
			var that = this;
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/xt_intro_msg/queryDataDetail",
				async: false,
				data : {
					'id':that.id,
				...
				success : function(result) {
					...
					that.introDetail=result.introDetail;
					that.celphone=result.celphone;
					that.address=result.address;
					that.email=result.email;
					setEditorHtml('introDetail',result.introDetail)
				}
			});
		},
		submitData(){
			...
			var introDetail = getEditVal('introDetail');
			var celphone = that.celphone;
			var address = that.address;
			var email = that.email;
			$.ajax({
				type : 'post',
				url : this.REQUEST_URL+"/admin/xt_intro_msg/update_submit",
				async: false,
				data : {
					'id':that.id,
					'title':title,
