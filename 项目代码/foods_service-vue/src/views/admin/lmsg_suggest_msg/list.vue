					...
					<el-table-column label="留言附图"  width="200px">
						...
						<el-table-column
							...
							<el-table-column
								...
								<template v-slot="{ row }">
									<el-button
										href="javascript:void(0)" :data-id="row.lmsgSuggestMsg.id" @click="del"
									...
									<template v-if="row.lmsgSuggestMsg.lmsgStatus=='1'">
										<el-button
											href="javascript:void(0)" :data-params="'id='+row.lmsgSuggestMsg.id+''" data-href="/admin/lmsg_suggest_msg/hf_page" @click="goToPage"
...
export default {
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/lmsg_suggest_msg/getInitData",
				async: false,
				data : {
		...
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/lmsg_suggest_msg/del",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/lmsg_suggest_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"title":title,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/lmsg_suggest_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
