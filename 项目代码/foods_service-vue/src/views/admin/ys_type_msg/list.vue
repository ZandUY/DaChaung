	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				<el-form-item label="类型名" style="min-width:250px;">
					<el-input type="text"  placeholder="请输入类型名" name="typeName" v-model="typeName" class="" id="typeName"></el-input>
				</el-form-item>
				<el-form-item>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/admin/ys_type_msg/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="类型名">
						<template v-slot="{ row }">
							<div v-html="''+(row.ysTypeMsg.typeName==null?'':row.ysTypeMsg.typeName)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="每千克热量">
						<template v-slot="{ row }">
							<div v-html="''+(row.ysTypeMsg.mqkrNum==null?'':row.ysTypeMsg.mqkrNum)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						fixed="right"
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-id="row.ysTypeMsg.id" @click="del"
								type="danger" style="margin-left:5px;margin-top:5px;"
								>
								删除
							</el-button>
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.ysTypeMsg.id+''" data-href="/admin/ys_type_msg/update_page" @click="goToPage"
				...
				<div>
					<div id="ys_type_msg_bar" class="pagination" style="margin-top:20px;"></div>
				</div>
			</div>
		</div>
	</el-card>
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/ys_type_msg/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/ys_type_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"typeName":typeName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/ys_type_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
