			...
			<div >
				<el-table
					...
					<el-table-column
						label="登录名">
						<template v-slot="{ row }">
							<div v-html="''+(row.userMsg.loginName==null?'':row.userMsg.loginName)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="姓名">
						<template v-slot="{ row }">
							<div v-html="''+(row.userMsg.name==null?'':row.userMsg.name)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="联系电话">
						<template v-slot="{ row }">
							<div v-html="''+(row.userMsg.celphone==null?'':row.userMsg.celphone)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="邮箱">
						<template v-slot="{ row }">
							<div v-html="''+(row.userMsg.email==null?'':row.userMsg.email)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="性别">
					...
					<el-table-column
						label="身高">
						<template v-slot="{ row }">
							<div v-html="''+(row.userMsg.sg==null?'':row.userMsg.sg)+'cm'"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="体重">
						<template v-slot="{ row }">
							<div v-html="''+(row.userMsg.weight==null?'':row.userMsg.weight)+'kg'"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="BMI">
						<template v-slot="{ row }">
							<div v-html="''+(row.userMsg.bmi==null?'':row.userMsg.bmi)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="年龄">
						<template v-slot="{ row }">
							<div v-html="''+(row.userMsg.age==null?'':row.userMsg.age)+''"></div>
						</template>
					</el-table-column>
					<el-table-column label="头像"  width="200px">
						...
						<el-table-column
							label="注册时间">
							<template v-slot="{ row }">
								<div v-html="''+(row.userMsg.zcTime==null?'':row.userMsg.zcTime)+''"></div>
							</template>
						</el-table-column>
						<el-table-column
							fixed="right"
							...
							<template v-slot="{ row }">
								<el-button
									href="javascript:void(0)" :data-id="row.userMsg.id" @click="del"
...
export default {
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/user_msg/getInitData",
				async: false,
				data : {
		...
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/user_msg/del",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/user_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"loginName":loginName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/user_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
