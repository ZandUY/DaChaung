	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/admin/yd_type_msg/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="类型名">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydTypeMsg.typeName==null?'':row.ydTypeMsg.typeName)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						label="每分钟消耗">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydTypeMsg.mfzxh==null?'':row.ydTypeMsg.mfzxh)+''"></div>
						</template>
					</el-table-column>
					<el-table-column
						fixed="right"
						...
						<template v-slot="{ row }">
							<el-button
								href="javascript:void(0)" :data-id="row.ydTypeMsg.id" @click="del"
								type="danger" style="margin-left:5px;margin-top:5px;"
								>
								删除
							</el-button>
							<el-button
								href="javascript:void(0)" :data-params="'id='+row.ydTypeMsg.id+''" data-href="/admin/yd_type_msg/update_page" @click="goToPage"
				...
				<div>
					<div id="yd_type_msg_bar" class="pagination" style="margin-top:20px;"></div>
				</div>
			</div>
		</div>
	</el-card>
	...
	methods:{
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/yd_type_msg/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/yd_type_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"typeName":typeName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/yd_type_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
