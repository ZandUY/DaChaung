	...
	<el-card class="box-card"  style="margin-left:2%;width:95%;margin-top:20px;">
		...
		<div>
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				...
				<el-form-item label="运动消耗范围" style="min-width:250px;">
			...
			<div style="margin-top:5px;padding-left: 15px;">
				<el-button
					href="javascript:void(0)" :data-params="''" data-href="/admin/ydjh_msg/add_page" @click="goToPage"
			...
			<div >
				<el-table
					...
					<el-table-column
						label="计划名">
						<template v-slot="{ row }">
							<div v-html="''+(row.ydjhMsg.jhName==null?'':row.ydjhMsg.jhName)+''"></div>
						</template>
					</el-table-column>
					<el-table-column label="封面图"  width="200px">
						...
						<el-table-column
							label="运动消耗范围">
							<template v-slot="{ row }">
								<div v-html="''+(row.ydxhfwStr==null?'':row.ydxhfwStr)+''"></div>
							</template>
						</el-table-column>
						<el-table-column
							label="发布时间">
							<template v-slot="{ row }">
								<div v-html="''+(row.ydjhMsg.fbTime==null?'':row.ydjhMsg.fbTime)+''"></div>
							</template>
						</el-table-column>
						<el-table-column
							fixed="right"
							...
							<template v-slot="{ row }">
								<el-button
									href="javascript:void(0)" :data-id="row.ydjhMsg.id" @click="del"
									type="danger" style="margin-left:5px;margin-top:5px;"
									>
									删除
								</el-button>
								<el-button
									href="javascript:void(0)" :data-params="'id='+row.ydjhMsg.id+''" data-href="/admin/ydjh_msg/update_page" @click="goToPage"
					...
					<div>
						<div id="ydjh_msg_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</el-card>
	...
	methods:{
		getInitData(){
			...
			$.ajax({
				...
				success : function(result) {
					that.ydxhfwMsgList = admin_utils.addAllSelect(result.ydxhfwMsgList);
		...
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/ydjh_msg/del",
					async: false,
					data : {
						"loginToken":that.currentLoginToken,
						"id":dataId
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/ydjh_msg/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
					"jhName":jhName,
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/ydjh_msg/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
